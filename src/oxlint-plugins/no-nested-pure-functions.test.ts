...
const createCallExpression = (calleeName: string): ESTree.CallExpression => {
...
const createExpressionStatement = (expression: ESTree.Expression): ESTree.ExpressionStatement => {
  const stmt = {
...
describe("no-nested-pure-functions", () => {
  it("should report named pure function inside another function", () => {
    const { report, visitor } = createRuleHarness(noNestedPureFunctionsRule, "no-nested-pure-functions");
    ...
    const innerFunction = createArrowFunctionExpression([], innerFunctionBody);

    // Create variable declaration for inner function
    const innerVarDecl = createVariableDeclaration("const", outerFunctionBody);
    const innerDeclarator = createVariableDeclarator("someotherfunc", innerFunction);
  ...
  it("should not report anonymous callback functions (like in .map or .filter)", () => {
    ...
    const callbackFunc = createArrowFunctionExpression([xParam as ESTree.BindingPattern], binaryExpr);
  ...
  it("should report named callback functions assigned to variables", () => {
    ...
    const mapperFunc = createArrowFunctionExpression([xParam as ESTree.BindingPattern], binaryExpr);
